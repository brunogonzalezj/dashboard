...
const Dashboard: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const url = `http://localhost:3001/data/${dataType === 'actual' ? 'dashboard-data' : 'history-dashboard-data'}`;
      ...
      <div className="overflow-x-auto">
        {!loading ? (
          <table className="min-w-full bg-white overflow-y-auto">
            <thead>
            <tr>
              {['course', 'name', 'lastName', 'email', 'business', 'stateOfCompleteness', 'progressPercentage'].map((column) => (
                <th
                  key={column}
            ...
            {displayedData.map((item) => (
              <tr key={item.id} className="text-center">
                ...
                <td className="py-2 px-4 border-b">
                  <div
                    className={`badge ${item.stateOfCompleteness === 'Completado' ? 'badge-success' : 'badge-error'} text-white`}>
